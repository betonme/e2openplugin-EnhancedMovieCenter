...
def parse_matroska_number(data, pos, unmodified=False, signed=False):
	...
	0x81 -> (1, pos+1)
	0x40,0x01 -> (1, pos+2)
	0x20,0x00,0x01 -> (1, pos+3)
	0x3F,0xFF,0xFF -> (0x1FFFFF, pos+3)
	0x20,0x00,0x01 unmodified -> (0x200001, pos+3)
	0xBF signed -> (0, pos+1)
	0xBE signed -> (-1, pos+1)
	0xC0 signed -> (1, pos+1)
	0x5F,0xEF signed -> (-16, pos+2)
    '''
    if unmodified and signed:
	raise Exception("Contradictary arguments")
    r = ord(data[pos])
...
def parse_xiph_number(data, pos):
	...
	0x01 -> (1,    pos+1)
	0x55 -> (0x55, pos+1)
	0xFF,0x04 -> (0x103,  pos+2)
	0xFF,0xFF,0x04 -> (0x202,  pos+3)
	0xFF,0xFF,0x00 -> (0x1FE,  pos+3)
    '''
    v = ord(data[pos])
    pos+=1
...
def parse_fixedlength_number(data, pos, length, signed=False):
	...
	0x01 -> (0x1,    pos+1)
	0x55 -> (0x55, pos+1)
	0x55 signed -> (0x55, pos+1)
	0xFF,0x04 -> (0xFF04,  pos+2)
	0xFF,0x04 signed -> (-0x00FC,  pos+2)
    '''
    r=0
...
def handle_block(buffer, handler, cluster_timecode, timecode_scale=1000000, duration=None, header_removal_headers_for_tracks={}):
	...
	buf = buffer[pos:]
	return handler.frame(tracknum, block_timecode, header_removal_prefix+buf, 0, duration, f_keyframe, f_invisible, f_discardable)
...
def mkvparse(f, handler):
	...
	if name=="EBML" and type(data) == list:
	    ...
	    if handler.segment_info_available():
		break

	    d = dict(tree)
	    if "TimecodeScale" in d:
		timecode_scale = d["TimecodeScale"][1]
	elif name=="Tags" and type(data) == list:
	    handler.tags_info = tree
	    if handler.tags_info_available():
		break
	...
	# cluster contents:
	    ...
	    data=f.read(size)
	    if handle_block(data, handler, current_cluster_timecode, timecode_scale, None,  header_removal_headers_for_tracks):
	      break
	    ...
	    if 'Block' in d2:
		if handle_block(d2['Block'][1], handler, current_cluster_timecode, timecode_scale, duration, header_removal_headers_for_tracks):
		  break
	else:
	    if type_!=EET.JUST_GO_ON and type_!=EET.MASTER:
		data = read_simple_element(f, type_, size)

	if handler.ebml_top_element(id_, name, type_, data):
	    break


class MatroskaTags(MatroskaHandler):
	...
	for (k,(t_,v)) in self.segment_info:
	    if t_ != EbmlElementType.TEXTU and t_ != EbmlElementType.TEXTA: continue
	    if k in ['Title', 'MuxingApp', 'WritingApp']:
		self.result[k.lower()] = v

    def tags_info_available(self):
	for (k,(t_,v)) in self.tags_info:
	    if k != 'Tag' or t_ != EbmlElementType.MASTER: continue
	    mtarget = 0
	    for (k2,(t2_,v2)) in v:
		if t2_ != EbmlElementType.MASTER: continue
		if k2 == 'Targets':
		    if not v2:
			mtarget = 1
		    else:
			for (k3,(t3_,v3)) in v2:
			    if k3 == 'TargetTypeValue' and int(v3) == 50:
				mtarget = 1
				break
		elif k2 == 'SimpleTag' and mtarget:
		    tagname = ''
		    d3 = {}
		    for (k3,(t3_,v3)) in v2:
			if t3_ != EbmlElementType.TEXTU and t3_ != EbmlElementType.TEXTA: continue
			if k3 == 'TagName':
			    tagname = v3
			    continue
			d3[k3] = v3
		    if 'TagLanguage' in d3:
			tagname += '@' + d3['TagLanguage']
			del d3['TagLanguage']
		    if len(d3) == 1 and 'TagString' in d3:
			d3 = d3['TagString']
		    if tagname and d3:
			self.result[tagname.lower()] = d3

    def frame(self, track_id, timestamp, data, more_laced_frames, duration, keyframe, invisible, discardable):
	return 1

def mkvtags(filename):
    try:
	parser = MatroskaTags()
	fp = open(filename, "rb")
	mkvparse(fp, parser)
	fp.close()
	res = {}
	for (key, val) in parser.result.iteritems():
	    a = key.split('@')
	    if len(a) > 1:
		if not a[0] in parser.result:
		    res[a[0]] = val
		    continue
	    res[key] = val
	return res
...
if __name__ == '__main__':
    ...
    pprint(mkvtags(sys.argv[1]), indent=1)
